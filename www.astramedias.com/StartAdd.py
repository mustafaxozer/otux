...
def print_output(
    ...
    client_disp = utils.get_display_name(client.get_me()).ljust(5)
    
    if isinstance(channel, str):
        channel_disp = channel.ljust(5)
    else:
        channel_disp = channel.username.ljust(5) if channel else 'None'.ljust(5)

    if isinstance(user, str):
        user_disp = user
    else:
        user_disp = utils.get_display_name(user).ljust(5) if user else 'None'.ljust(5)

    print(f"{client_disp} | {channel_disp} | {user_disp} | {status.ljust(5)}")
...
if __name__ == '__main__':
    ...
    try:
        ...
        user_rows = read_csv('MembersFile.csv')
        ...
        try:
            for api in api_info_list:
                print(f"Thankas For Connecting Us\n")
